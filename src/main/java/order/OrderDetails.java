	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(ck!=null && ck[0].getName().equals("name")) {
			...
			try {
				...
				//if(rs.next()) {
					out.println("<table border=1 width=100% height=10% border-collapse=collapse> <tr style='background-color:cyan; color:green'> <th>Order Id</th> <th>Product Name</th> <th>Quantity</th> <th>Total price</th> <th>Address</th>  <th>Order Date</th> </tr>");
					out.print("<h2 style='padding-left:45%;padding-right:29%;'> Order Details </h2>");
					while(rs.next()) {
				
						out.print("<tr style='background-color:orange;'> <td>"+rs.getString("order_id")+"</td>");
