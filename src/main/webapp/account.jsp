...
body{background-image:url("images/acc5.jpg");background-repeat:no-repeat;background-size:cover;}

div{
width: 410px;background-repeat:no-repeat;background-size:cover; font-size:25px;
padding: 50px;margin: 20px;display:block; border-radius: 10px;margin-left:30%;margin-top:10%; color: white; font-weight:bold;}
tr,td{padding:10px;}
table{padding:10px;}
...
function logoutConfirm(){
	if (confirm("log out, confirm?!")) {
		window.location.replace("http://localhost:8090/shoeshop/LogOut");
	  } 
}

function deleteConfirm(){
	if (confirm("delete account, confirm?!")) {
		window.location.replace("http://localhost:8090/shoeshop/DeleteAccount");
	  } 
}
